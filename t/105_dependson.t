...
{
  # Regression test that verifies we can ask for dependencies on objects with properties not set
  my $s = Cfn->from_json('{"Resources" : {"IAMUser" : {"Type" : "AWS::IAM::User"} }}');
  is_deeply($s->Resource('IAMUser')->dependencies, [], 'Can ask for deps on an resource with no Properties');
}
